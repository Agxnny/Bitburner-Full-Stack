...
export async function main(ns){
 ...
 try{
  ...
  ex(ns,requestExecution({requestId:tag+":bad",executionId:tag+":bad",workOrderId:orderId,receiver:"wrong-receiver",correlationId:tag,script:FIXTURE,threads:1,args:[750],ttlMs:10000}));
  ...
  ex(ns,requestExecution({requestId:tag+":run",executionId,workOrderId:orderId,receiver,correlationId:tag,script:FIXTURE,threads:1,args:[750],ttlMs:10000}));
...
function auth(ns,c){if(!publishAuthorityCommand(ns,c))throw new Error("Authority queue full.");} function wo(ns,c){if(!publishWorkOrderCommand(ns,c))throw new Error("Work Order queue full.");} function ex(ns,c){if(!publishExecutionCommand(ns,c))throw new Error("Execution queue full.");}
