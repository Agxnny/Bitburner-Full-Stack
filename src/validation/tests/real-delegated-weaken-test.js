...
export async function main(ns){
 ...
 try{
  target=chooseTarget(read(ns,"data/state/network.json"),ns.getHackingLevel());
  ...
  auth(ns,acquireAuthority({requestId:tag+":grant",leaseId,owner:issuer,intent:"validation-real-weaken",claims:[claim],ttlMs:120000,correlationId:tag}));
  ...
  wo(ns,createWorkOrder({requestId:tag+":create",workOrderId:orderId,issuer,receiver,objective:"weaken-once",claims:[claim],authorityLeaseId:leaseId,correlationId:tag,constraints:{operation:"weaken",threads:1,target:target.hostname},ttlMs:90000,cleanupTtlMs:3000}));
  ...
  ns.rm(resultPath,"home");pid=ns.run(EXECUTOR,1,orderId,receiver,target.hostname,resultPath);check(a,"executor-started",pid>0,"Started one-thread temporary delegated executor pid "+pid+".");if(pid<=0)throw new Error("Executor failed to start.");
  const result=await wait(ns,()=>read(ns,resultPath),Math.min(90000,Math.max(10000,ns.getWeakenTime(target.hostname)+5000)));
  check(a,"delegated-execution",result?.status==="COMPLETED"&&result?.authorization?.mode==="DELEGATED","Executor with no direct lease passed DELEGATED authorization immediately before weaken.");
  check(a,"real-weaken",Number.isFinite(result?.before)&&Number.isFinite(result?.after)&&result.after<result.before,"Real weaken changed security from "+(result?.before??"?")+" to "+(result?.after??"?")+".");
  ...
  auth(ns,releaseAuthority({requestId:tag+":release",leaseId,owner:issuer}));await authDecision(ns,tag+":release",4000);await ns.sleep(250);
  check(a,"cleanup-clean",!read(ns,AUTHORITY_STATE_PATH)?.leases?.some(x=>x.leaseId===leaseId)&&findOrder(ns,orderId)?.state==="CLOSED"&&!ns.isRunning(pid,"home"),"No live test lease or executor remains; Work Order is terminal CLOSED.");
...
function chooseTarget(network,hackingLevel){if(!network||network.kind!=="canonical-state"||network.domain!=="network"||network.availability!=="available"||!Array.isArray(network.data?.servers))return null;return network.data.servers.filter(s=>s.hostname!=="home"&&s.hasAdminRights&&!s.purchasedByPlayer&&s.isOnline!==false&&s.requiredHackingSkill<=hackingLevel&&Number.isFinite(s.hackDifficulty)&&Number.isFinite(s.minDifficulty)&&s.hackDifficulty>s.minDifficulty+0.0001).sort((a,b)=>(b.hackDifficulty-b.minDifficulty)-(a.hackDifficulty-a.minDifficulty)||a.hostname.localeCompare(b.hostname))[0]??null;}
