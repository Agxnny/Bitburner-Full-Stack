...
export function workOrder({workOrderId,issuer,receiver,objective,claims,authorityLeaseId,correlationId,constraints=null,createdAt=Date.now()}){
    return {schemaVersion:1,kind:"work-order",workOrderId,issuer,receiver,objective,claims,authorityLeaseId,correlationId,constraints,state:"PENDING",createdAt,updatedAt:createdAt,expiresAt:null};
}
...
export function createWorkOrder({requestId,workOrderId,issuer,receiver,objective,claims,authorityLeaseId,correlationId,constraints=null,ttlMs,at=Date.now()}){
    return {schemaVersion:1,kind:"work-order-command",action:"create",requestId,workOrderId,issuer,receiver,objective,claims,authorityLeaseId,correlationId,constraints,ttlMs,at};
}
...
export function validWorkOrderCommand(v){
    if(!v||v.schemaVersion!==1||v.kind!=="work-order-command"||!["create","close","cancel"].includes(v.action))return false;
